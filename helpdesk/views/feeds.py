...
class OpenTicketsByUser(Feed):
    ...
    def link(self, obj):
        if obj['queue']:
            return u'{}?assigned_to={}&queue={}'.format(reverse('helpdesk:ticket-list'), obj['user'].id,
                                                        obj['queue'].id)
        else:
            return u'%s?assigned_to=%s' % (
                reverse('helpdesk:ticket-list'),
                obj['user'].id,
            )
...
class UnassignedTickets(Feed):
    ...
    def link(self, obj):
        return '{}?no_assigned=True'.format(reverse('helpdesk:ticket-list'))
...
class OpenTicketsByQueue(Feed):
    ...
    def link(self, obj):
        return '{}?queue={}&status={}&status={}'.format(
            reverse('helpdesk:ticket-list'), obj.id, Ticket.OPEN_STATUS, Ticket.REOPENED_STATUS)
